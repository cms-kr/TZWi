...
bool ttbarDoubleLeptonCppWorker::analyze() {
  ...
  for ( unsigned i=0, n=in_Electrons_p4[0]->GetSize(); i<n; ++i ) {
    ...
    if ( isVetoElectron(i) ) {
      if ( vetoElectron3Idx < 0 or pt > in_Electrons_p4[0]->At(vetoElectron3Idx) ) vetoElectron3Idx = i;
      if ( vetoElectron2Idx < 0 or pt > in_Electrons_p4[0]->At(vetoElectron2Idx) ) std::swap(vetoElectron2Idx, vetoElectron3Idx);
      if ( vetoElectron1Idx < 0 or pt > in_Electrons_p4[0]->At(vetoElectron1Idx) ) std::swap(vetoElectron1Idx, vetoElectron2Idx);
    }
  }
