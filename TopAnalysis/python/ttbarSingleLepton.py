...
class ttbarSingleLeptonEvent(Module, object):
    def __init__(self, *args, **kwargs):
        #super(ttbarSingleLeptonEvent, self).__init__(*args, **kwargs)
        self.mode = kwargs.get("mode")
        self.algo = kwargs.get("algo")

        if "/ttbarSingleLepton_cc.so" not in  ROOT.gSystem.GetLibraries():
            print "Load C++ ttbarSingleLepton worker module"
        ...
        pass
    def beginJob(self):
        self.worker = ROOT.ttbarSingleLeptonCppWorker(self.mode, self.algo)
        pass
    def endJob(self):
        pass
...
ttbarSingleLepton = lambda : ttbarSingleLeptonEvent(mode="Any", algo="CloseMTop")
