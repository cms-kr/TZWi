...
class FCNCKinematicReco(Module, object):
    ...
    def analyze(self, event):
        ...
        Wlepvar = [event._tree.b_out_Lepton1_pt, event._tree.b_out_Lepton1_eta, event._tree.b_out_Lepton1_phi, event._tree.b_out_Lepton1_mass]
        ...
        jet1var = [event._tree.b_out_GoodJet_pt[0], event._tree.b_out_GoodJet_eta[0], event._tree.b_out_GoodJet_phi[0], event._tree.b_out_GoodJet_mass[0], event._tree.b_out_GoodJet_CSVv2[0]]
        jet2var = [event._tree.b_out_GoodJet_pt[1], event._tree.b_out_GoodJet_eta[1], event._tree.b_out_GoodJet_phi[1], event._tree.b_out_GoodJet_mass[1], event._tree.b_out_GoodJet_CSVv2[1]]
        # Variable condtruct : Neutrino vars = [MET, phi]
        metvar = [event._tree.b_out_MET_pt, event._tree.b_out_MET_phi]

        # b jet assign by CSVv2 discriminator
        if ( jet1var[4] > jet2var[4] ): 
            bvar = jet1var
            qvar = jet2var
        else:
            bvar = jet2var
        ...
        Zlep1 = self.getKinVar(Zlep1var[0], Zlep1var[1], Zlep1var[2], 0.)
        Zlep2 = self.getKinVar(Zlep2var[0], Zlep2var[1], Zlep2var[2], 0.)
        Wlep = self.getKinVar(Wlepvar[0], Wlepvar[1], Wlepvar[2], 0.) ## Assume lepton mass = 0
        bjet = self.getKinVar(bvar[0], bvar[1], bvar[2], bvar[3])
        qjet = self.getKinVar(qvar[0], qvar[1], qvar[2], qvar[3])
        
        # MET pxy calculation & construction : [px, py]
        metpx = metvar[0]*(math.cos(metvar[1]))
        metpy = metvar[0]*(math.sin(metvar[1]))
        met = [metpx, metpy]
